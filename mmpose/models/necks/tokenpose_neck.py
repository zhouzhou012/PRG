...
class TokenPose_TB_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False, apply_multi=True, hidden_heatmap_dim=64*6,heatmap_dim=64*48,heatmap_size=[64,48], channels, dropout = 0., emb_dropout = 0., pos_embedding_type="learnable"):
        super().__init__()
        ...
        #     nn.LayerNorm(dim),   #192
        #     nn.Linear(dim, hidden_heatmap_dim),  #192->384
        #     nn.LayerNorm(hidden_heatmap_dim),
        #     nn.Linear(hidden_heatmap_dim, heatmap_dim) #384->64*48
        ...
        # self.mlp_head = nn.Sequential(
        #     nn.LayerNorm(dim),  # 192
        #     nn.Linear(dim, 192),  # 192->192
        #     nn.LayerNorm(192),
        #     nn.Linear(192,4) #192->4
        #     )
        ...
        # Define SimCC layers
        # flatten_dims =64 * 48  # 64*48=3072
        # W = int(192 * 2.0)  # 192*2=384
        # H = int(256 * 2.0)  # 256*2=512
        # self.mlp_head_x = nn.Linear(flatten_dims, W)
        # self.mlp_head_y = nn.Linear(flatten_dims, H)

        # flatten_dims = 192  # 192
        # W = int(192 * 2.0)  # 192*2=384
        # H = int(256 * 2.0)  # 256*2=512
        # self.mlp_head_x = nn.Linear(flatten_dims, W)
        # self.mlp_head_y = nn.Linear(flatten_dims, H)
