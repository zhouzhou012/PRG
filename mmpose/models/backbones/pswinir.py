...
class PSwinIR(BaseBackbone):
    ...
    def init_weights(self):
        ...
        super(PSwinIR, self).init_weights()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x torch.Size([B, C, H,W])
        # # # 使用permute交换维度
        # out_permuted = x.permute(0, 2, 3, 1)  # torch.Size([B,H,W,C])
        # bgr_image0 = cv2.cvtColor(to_numpy(out_permuted[0]), cv2.COLOR_RGB2BGR)
        # cv2.imwrite('/mnt/private/mmpose/input.png', bgr_image0)
        x = torch.tensor((x / 255.).clip(0, 1), dtype=torch.float32)
        H, W = x.shape[2:]
        # 检查图片的大小，使高宽满足 window_size 的整数倍,否则会填充x
        x = self.check_image_size(x)

        self.mean = self.mean.type_as(x)  # RGB 均值的类型同 x 一致
        x = (x - self.mean) * self.img_range  #归一化［0,1] 或[0,255]

        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x) #torch.Size([1, 180, 64, 48])
            #stem
            # x = self.conv1(x)
            # x = self.bn1(x)
            # x = self.relu(x)
            # x = self.maxpool(x) #torch.Size([1, 180, 64, 48])
            #残差结构
            # torch.Size([B, embed_dim, 64, 48])
            x = self.conv_after_body(self.forward_features(x)) + x
            # x = self.conv_after_body(self.forward_features(x))
            feats=self.net(x)
            x = self.conv_before_upsample(x) #torch.Size([B, num_feat, 64, 48])
            x = self.conv_last(self.upsample(x)) #torch.Size([B, 3, 256, 192])
            ...
            x = self.bn1(x)
            x = self.relu(x)
            x = self.maxpool(x)  # torch.Size([1, 180, 64, 64])

            # 残差结构
            # x = self.conv_after_body(self.forward_features(x)) + x
            x = self.conv_after_body(self.forward_features(x))
            res = x  # torch.Size([B, embed_dim, 64, 64])
