...
class PCKAccuracy(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        # stats = {
        #     'Nose PCK': PCK[0],
        #     'Eyes PCK': 0.5 * (PCK[1] + PCK[2]),
        #     'Ears PCK': 0.5 * (PCK[3] + PCK[4]),
        #     'Shoulder PCK': 0.5 * (PCK[5] + PCK[6]),
        #     'Elbow PCK': 0.5 * (PCK[7] + PCK[8]),
        #     'Wrist PCK': 0.5 * (PCK[9] + PCK[10]),
        #     'Hip PCK': 0.5 * (PCK[11] + PCK[12]),
        #     'Knee PCK': 0.5 * (PCKh[13] + PCK[14]),
        #     'Ankle PCK': 0.5 * (PCK[15] + PCK[16]),
        #     'PCK': PCK
        # }
        #
