...
class Distill_TokenPose_TB_base(nn.Module):
    def __init__(self, *, heatmap_size=[64, 48], feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim,
                 apply_init=True, channels, dropout=0., emb_dropout=0., pos_embedding_type="learnable"):
        super().__init__()
        ...
        self.distill_token = nn.Parameter(torch.zeros(1, self.num_keypoints, dim))
        ...
        self.to_keypoint_token = nn.Identity()  # B 17 192
    ...
    def forward(self, feature, mask=None):
        ...
        if self.pos_embedding_type in ["sine", "sine-full"]:
            ...
            x = torch.cat((keypoint_tokens,distill_tokens, x), dim=1)
        else:
        ...
        kpt_token = self.to_keypoint_token(x[:, 0:self.num_keypoints])
        dis_token = self.to_keypoint_token(x[:, self.num_keypoints:self.num_keypoints*2])
        vis_token = x[:, self.num_keypoints*2:]

        # mlp head
        # x = self.mlp_head(x)  #batch_size, k, w * h   ->  #batch_size 17 64*48
        # pred_x = self.mlp_head_x(x)  # torch.Size([B, 17, 384])
        # pred_y = self.mlp_head_y(x)  # torch.Size([B, 17, 512])
        # return pred_x, pred_y  # 一维向量表征
        # x = rearrange(x,'b c (p1 p2) -> b c p1 p2',p1=self.heatmap_size[0],p2=self.heatmap_size[1])
        return (kpt_token,dis_token,vis_token)
