...
class CPEN(nn.Module):
    def __init__(self,n_feats = 64, n_encoder_res = 6):
        ...
        E1=[nn.Conv2d(192, n_feats, kernel_size=3, padding=1),
        ...
        # plan2 不如plan1
        # E3 = [
        #     nn.Conv2d(n_feats, 32, kernel_size=3, padding=1),
        #     nn.LeakyReLU(0.1, True),
        ...
        #
        # E4 = [
        #     nn.Conv2d(n_feats*2, 32, kernel_size=3, padding=1),
        #     nn.LeakyReLU(0.1, True),
        #     nn.Conv2d(32, 16, kernel_size=3, padding=1),
        #     nn.LeakyReLU(0.1, True),
        #     nn.Conv2d(16, 1, kernel_size=3, padding=1),
        ...
        #
        # E4 = [
        #     nn.Conv2d(n_feats*4, 32, kernel_size=3, padding=1),
        #     nn.LeakyReLU(0.1, True),
        #     nn.Conv2d(32, 16, kernel_size=3, padding=1),
        #     nn.LeakyReLU(0.1, True),
        #     nn.Conv2d(16, 1, kernel_size=3, padding=1),
    ...
    #
    #     prior = self.mlp(fea)
    #     return prior  #(B,256)  #(B,1,64,48)

@MODELS.register_module()
class Prior_TopdownPoseEstimator_Stage1(BaseModel, metaclass=ABCMeta):
    _version = 2
    ...
    def predict(self, inputs: Tensor, data_samples: SampleList) -> SampleList:
        ...
        # file_name = "/mnt/private/mmpose/vis_results/hr_inputs"
        # for i in range(hr_img.size(0)):
        #       output_file = os.path.join(file_name, f"image_{i}.png")
        #       cv2.imwrite(output_file, to_numpy(hr_img[i]))
        ...
        #     plt.axis('off')
        #     output_file = '/mnt/private/mmpose/mask1.png'
        #     plt.savefig(output_file)



        #
        # file_name = "/mnt/private/mmpose/vis_results/mask"
        # for k in range(prior.size(0)):
        #     vis = prior[k].squeeze(-1)
        #     vis = rearrange(vis, " c h w ->h w c").contiguous()
        #     # # 保存张量到文件
        #     # vis = to_numpy(vis)
        #     # np.save('tensor1.npy', vis0)
        #
        #     vis1 = to_numpy((torch.sigmoid(vis * vis)))
        #     plt.imshow(vis1.squeeze(-1), cmap='viridis')
        #     plt.axis('off')
        #     output_file = os.path.join(file_name, f"mask_{k}.png")
        #     plt.savefig(output_file)
