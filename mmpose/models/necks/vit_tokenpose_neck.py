...
class FeatureMapProcessor_TokenPose_TB_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False,heatmap_size=[64,48], channels, dropout = 0., emb_dropout = 0., pos_embedding_type="learnable",scale_factor,apply_relu):
        super().__init__()
        ...
        self.FeatureMapProcessor_neck = self._make_neck(
    ...
    def _make_neck(
       ...
       FeatureMapProcessor_neck = MODELS.build(
                dict(
                     type='FeatureMapProcessor',
                     scale_factor=scale_factor,
                     apply_relu=apply_relu))
       return FeatureMapProcessor_neck
    ...
    def forward(self, feature, mask = None):
        p = self.patch_size
        #　先将16 12插值成64 48
        feature=self.FeatureMapProcessor_neck(feature)  #torch.Size([1, 32, 768, 64, 48])

        # 使用 squeeze 函数去除第一个维度的大小为1的维度
        feature = feature[-1]
