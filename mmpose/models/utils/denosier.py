...
class Unet(nn.Module):
    def __init__(
        ...
        self.channels = channels  #17
        self.self_condition = self_condition
        # input_channels = channels * (2 if self_condition else 1)
        input_channels = 32+17  #特殊处理(fea+x_t)
        init_dim = default(init_dim, dim)  #dim
    ...
    def forward(self, x, x_self_cond,time):
...
class Unet2(nn.Module):
    def __init__(
        ...
        self.self_condition = self_condition
        # input_channels = channels * (2 if self_condition else 1)
        input_channels = 32+channels+channels  #特殊处理x_c=32+17  x_t 17  32+14+14
        init_dim = default(init_dim, dim)  #dim
        self.init_conv = nn.Conv2d(input_channels, init_dim, 7, padding = 3)
        dims = [init_dim, *map(lambda m: dim * m, dim_mults)]
        in_out = list(zip(dims[:-1], dims[1:]))
...
class Unet3(nn.Module):  #Res_diff
    def __init__(
        self,
        dim,
        init_dim = None,
        ...
        self.self_condition = self_condition
        # input_channels = channels * (2 if self_condition else 1)
        input_channels =66   #17+17+32　（x_t,x_input,feats[-1])
        init_dim = default(init_dim, dim)  #dim=64
        self.init_conv = nn.Conv2d(input_channels, init_dim, 7, padding = 3)
        dims = [init_dim, *map(lambda m: dim * m, dim_mults)]
        in_out = list(zip(dims[:-1], dims[1:]))
    ...
    def forward(self, x, x_self_cond,time):
        if self.num_unet == 2:
            if self.test_res_or_noise == "res_noise":
                return self.unet0(x, x_self_cond,time[0]), self.unet1(x,x_self_cond,time[1], )
            elif self.test_res_or_noise == "res":  #选这个
                return self.unet0(x, x_self_cond,time[0]), 0
            elif self.test_res_or_noise == "noise":
                return 0, self.unet1(x, x_self_cond,time[1])
        elif self.num_unet == 1:
            if self.objective == 'pred_res_noise':
                ...
                time = time[0]
            return [self.unet0(x, x_self_cond,time)]
