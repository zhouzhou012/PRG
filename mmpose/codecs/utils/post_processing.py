...
def get_simcc_soft_maximum(simcc_x: np.ndarray,
                      simcc_y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    assert isinstance(simcc_x, np.ndarray), ('simcc_x should be numpy.ndarray')
    assert isinstance(simcc_y, np.ndarray), ('simcc_y should be numpy.ndarray')
    assert simcc_x.ndim == 2 or simcc_x.ndim == 3, (
    ...
    #np.ndarray转为tensor   simcc_xx tensor   simcc_x  np.ndarray
    simcc_xx = torch.tensor(simcc_x)
    simcc_yy = torch.tensor(simcc_y)

    min_vals_x, _ = torch.min(simcc_xx, dim=1)
    max_vals_x, _ = torch.max(simcc_xx, dim=1)

    min_vals_y, _ = torch.min(simcc_yy, dim=1)
    max_vals_y, _ = torch.max(simcc_yy, dim=1)

    # 对 tensor 张量进行最大最小归一化
    simcc_xx = ((simcc_xx - min_vals_x[:, None]) / (max_vals_x[:, None] - min_vals_x[:, None]))*10.0
    simcc_yy = ((simcc_yy - min_vals_y[:, None]) / (max_vals_y[:, None] - min_vals_y[:, None]))*10.0


    #(N,W)或者（N×K，W）扩充
    simcc_xx = simcc_xx.unsqueeze(1)  # (N*K,1,W)
    simcc_yy = simcc_yy.unsqueeze(-1)  # (N×K,H,1)

    W=simcc_xx.shape[-1]
    H=simcc_yy.shape[1]


    #可以获得一个网格，在接下来的计算中可以使用它来生成坐标
    linspace_x = torch.arange(0.0, 1.0 * W, 1).reshape(1, 1, W) / W
    linspace_y = torch.arange(0.0, 1.0 * H, 1).reshape(1, H, 1) / H

    #将linspace_x和linspace_y转换为可训练的参数nn.Parameter，并设置requires_grad=False，表示这些参数不需要梯度计算
    linspace_x = nn.Parameter(linspace_x, requires_grad=False)
    linspace_y = nn.Parameter(linspace_y, requires_grad=False)

    feats_x=simcc_xx
    feats_y=simcc_yy

    #  softmax 归一化
    simcc_xx = F.softmax(simcc_xx*1.0, dim=-1)
    simcc_yy = F.softmax(simcc_yy*1.0, dim=1)


    #计算加权平均位置
    x_locs = simcc_xx.mul(linspace_x).reshape(simcc_xx.shape[0],-1)
    x_locs= torch.sum(x_locs, dim=1, keepdim=True)


    y_locs = simcc_yy.mul(linspace_y).reshape(simcc_yy.shape[0],-1)
    y_locs = torch.sum(y_locs, dim=1, keepdim=True)

    # 去偏差
    Cx = feats_x.reshape(N*K, W).exp().sum(dim=1).reshape(N*K, 1)
    Cy = feats_y.reshape(N*K, H).exp().sum(dim=1).reshape(N*K, 1)
    x_locs = Cx / (Cx - 1) * (x_locs - 1 / (2 * Cx))
    y_locs= Cy / (Cy - 1) * (y_locs - 1 / (2 * Cy))

    locs = torch.cat([x_locs,y_locs], dim=-1)
    max_val_x = np.amax(simcc_x, axis=1)
    max_val_y = np.amax(simcc_y, axis=1)
