...
from .Dynamic_resnet import BasicBlock, Bottleneck, get_expansion
...
class HRModule(BaseModule):
    ...
    def forward(self, inputs):
        ...
        k_v = inputs[1]
        if self.num_branches == 1:
            return [[self.branches[0]([x[0], k_v])], k_v]

        for i in range(self.num_branches):
            x[i],_ = self.branches[i]([x[i],k_v])
...
class Dynamic_HRNet(BaseBackbone):
    """HRNet backbone.
    ...
    def init_weights(self):
        ...
        super(Dynamic_HRNet, self).init_weights()
    ...
    def forward(self, x,k_v):
        ...
        for i in range(self.stage2_cfg['num_branches']):
            if self.transition1[i] is not None:
                ...
                x_list.append(x)
        y_list,_ = self.stage2([x_list,k_v])
        ...
        y_list,_ = self.stage3([x_list,k_v])
        ...
        for i in range(self.stage4_cfg['num_branches']):
            if self.transition3[i] is not None:
                ...
                x_list.append(y_list[i])
        y_list,_ = self.stage4([x_list,k_v])
        return tuple(y_list)
